...
function snapFixture(takenAt: string, views: number[]): NormalizedSnapshot {
  ...
  return {
    ...
    videos: [
      { platform: "tiktok", platformVideoId: "1", title: "Viral one", metrics: { views: 144941, likes: 9000, comments: 800, shares: 700 } },
      { platform: "tiktok", platformVideoId: "2", title: "Quiet one", metrics: { views: 850, likes: 40, comments: 3, shares: 2 } },
    ],
    audience: [{ dimension: "country", key: "Nepal", value: 0.68 }],
    warnings: [],
  };
}
...
test("storeSnapshot + credentials roundtrip + compare_periods", withVault(async (vault) => {
  const acct = vault.upsertAccount({ platform: "tiktok", platformAccountId: "open_id_1", handle: "sadikbee" });
...
test("top_content sorts by views and engagement_rate", withVault((vault) => {
  ...
  assert.equal(top[0].title, "Viral one");
  ...
  assert.equal(byRate[0].title, "Viral one");
}));
...
test("digest + media kit read from vault", withVault((vault) => {
  const acct = vault.upsertAccount({ platform: "tiktok", platformAccountId: "x", handle: "sadikbee" });
  ...
  assert.equal(kit.accounts[0].handle, "sadikbee");
  assert.equal(kit.accounts[0].followers, 1084);
  assert.equal(kit.audienceHighlights.find((a) => a.key === "Nepal")?.value, 0.68);

  const aud = audienceOverview(vault);
  assert.equal(aud.find((a) => a.dimension === "country")?.key, "Nepal");
}));
